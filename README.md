...
| <img src="./plugins/host-monitor/assets/icon.svg" width="128" height="128" alt="" /> | [Host Monitor](./plugins/host-monitor) | [Git release](#host-monitor-quick-start) | Adds a Grafana-inspired in-page fleet dashboard with current and historical CPU, RAM, disk, load, and network telemetry for every BB machine. Requires BB 0.40+. |
...
Host Monitor keeps every enrolled machine in one responsive fleet overview.
Select a machine card for current stats and up to 30 days of utilization, load,
and network history. Its branded sidebar row shows only neutral connected/total
text and opens the page normally—there are no popups, floating windows, warning
badges, toasts, or desktop/browser notifications. See the
