...
Ship reviewable fixes that make Save My Model useful on BB 0.42.1 through
...
- [ ] [AC-4] Save My Model documentation and marketplace metadata describe the behavior actually shipped on BB 0.42.1 and do not claim to override BB's built-in New Thread picker.
...
Use focused unit and frontend-harness tests, exact SDK type checks, production plugin builds, package/managed-install checks, the root workspace check, and a real BB 0.42.1 browser exercise with at least two hosts including a disconnected host. Inspect the final default-branch-relative diff and current marketplace CI. Capture screenshots for each changed or newly submitted visible surface.
